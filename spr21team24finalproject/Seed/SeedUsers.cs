using System;
using System.Collections.Generic;
using Microsoft.AspNetCore.Identity;

//TODO: Update these using statements to include your project name
using spr21team24finalproject.Models;
using spr21team24finalproject.Utilities;
using spr21team24finalproject.DAL;
using System.Threading.Tasks;

//TODO: Upddate this namespace to match your project name
namespace spr21team24finalproject.Seeding
{
    public static class SeedUsers
    {
        public async static Task<IdentityResult> SeedAllUsers(UserManager<AppUser> userManager, AppDbContext context)
        {
            //Create a list of AddUserModels
            List<AddUserModel> AllUsers = new List<AddUserModel>();

            //Customer Seed Data
            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "cbaker@freezing.co.uk",
                    Email = "cbaker@freezing.co.uk",
                    PhoneNumber = "5125595133",
                    FirstName = "Christopher",
                    LastName = "Baker",
                    Address = "1245 Lake America Blvd.",
                    ZipCode = "78733"
                },
                Password = "hellothere",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "mb@puppy.com",
                    Email = "mb@puppy.com",
                    PhoneNumber = "2102702860",
                    FirstName = "Michelle",
                    LastName = "Bradicus",
                    Address = "1300 Small Pine Lane",
                    ZipCode = "78261"
                },
                Password = "555533",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "fd@puppy.com",
                    Email = "fd@puppy.com",
                    PhoneNumber = "8175683686",
                    FirstName = "Franco",
                    LastName = "Broccoli",
                    Address = "62 Cookie Rd",
                    ZipCode = "77019"
                },
                Password = "666645",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "wendy@puppy.com",
                    Email = "wendy@puppy.com",
                    PhoneNumber = "5125967209",
                    FirstName = "Wendy",
                    LastName = "Charile",
                    Address = "202 Bellmoth Hall",
                    ZipCode = "78713"
                },
                Password = "Kansas",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "limchou@puppy.com",
                    Email = "limchou@puppy.com",
                    PhoneNumber = "2107748586",
                    FirstName = "Lim",
                    LastName = "Chou",
                    Address = "1600 Barbara Lane",
                    ZipCode = "78266"
                },
                Password = "Rockwall",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "444444.Dave@aool.com",
                    Email = "444444.Dave@aool.com",
                    PhoneNumber = "2142667242",
                    FirstName = "Shan",
                    LastName = "Dave",
                    Address = "234 Puppy Circle",
                    ZipCode = "75208"
                },
                Password = "444444",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "louann@puppy.com",
                    Email = "louann@puppy.com",
                    PhoneNumber = "8172580736",
                    FirstName = "Lou Ann",
                    LastName = "Feeley",
                    Address = "700 S 9th Street W",
                    ZipCode = "77010"
                },
                Password = "longhorns",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "tfreeley@puppy.com",
                    Email = "tfreeley@puppy.com",
                    PhoneNumber = "8173279674",
                    FirstName = "Tesa",
                    LastName = "Freeley",
                    Address = "4334 Meanview Ave.",
                    ZipCode = "77009"
                },
                Password = "puppies",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "mgar@puppy.com",
                    Email = "mgar@puppy.com",
                    PhoneNumber = "8176617531",
                    FirstName = "Margaret",
                    LastName = "Garcia",
                    Address = "594 Puppyview",
                    ZipCode = "77003"
                },
                Password = "horses",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "chaley@thug.com",
                    Email = "chaley@thug.com",
                    PhoneNumber = "2148499570",
                    FirstName = "Charles",
                    LastName = "Harley",
                    Address = "One Ranger Pkwy",
                    ZipCode = "75261"
                },
                Password = "mycats",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "jeff@puppy.com",
                    Email = "jeff@puppy.com",
                    PhoneNumber = "5127002600",
                    FirstName = "Jeffrey",
                    LastName = "Stark",
                    Address = "337 40th St.",
                    ZipCode = "78705"
                },
                Password = "jeffery",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "wjhearniii@umch.edu",
                    Email = "wjhearniii@umch.edu",
                    PhoneNumber = "2148989608",
                    FirstName = "John",
                    LastName = "Hearn",
                    Address = "4445 South First",
                    ZipCode = "75237"
                },
                Password = "posicles",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "hicks43@puppy.com",
                    Email = "hicks43@puppy.com",
                    PhoneNumber = "2105812952",
                    FirstName = "Mark",
                    LastName = "Hicks",
                    Address = "32 NE Mark Ln., Ste 910",
                    ZipCode = "78239"
                },
                Password = "guac45",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "bradsingram@mall.utexas.edu",
                    Email = "bradsingram@mall.utexas.edu",
                    PhoneNumber = "5124702808",
                    FirstName = "Brad",
                    LastName = "Ingram",
                    Address = "6548 La Chess St.",
                    ZipCode = "78736"
                },
                Password = "father",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "father.Ingram@aool.com",
                    Email = "father.Ingram@aool.com",
                    PhoneNumber = "5124677352",
                    FirstName = "Todd",
                    LastName = "Jacobs",
                    Address = "4564 Palm St.",
                    ZipCode = "78731"
                },
                Password = "555897",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "victoria@puppy.com",
                    Email = "victoria@puppy.com",
                    PhoneNumber = "5129481386",
                    FirstName = "Victoria",
                    LastName = "Lawrence",
                    Address = "6639 Butterfly Ln.",
                    ZipCode = "78761"
                },
                Password = "something",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "lineback@flush.net",
                    Email = "lineback@flush.net",
                    PhoneNumber = "2102473963",
                    FirstName = "Brad",
                    LastName = "Lineback",
                    Address = "1300 Pirateland St",
                    ZipCode = "78293"
                },
                Password = "treelover",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "elowe@netscrape.net",
                    Email = "elowe@netscrape.net",
                    PhoneNumber = "2105368614",
                    FirstName = "Evan",
                    LastName = "Lowe",
                    Address = "3201 Pineapple Drive",
                    ZipCode = "78279"
                },
                Password = "headear",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "luce_chuck@puppy.com",
                    Email = "luce_chuck@puppy.com",
                    PhoneNumber = "2107007535",
                    FirstName = "Chuck",
                    LastName = "Luce",
                    Address = "2345 Silent Clouds",
                    ZipCode = "78268"
                },
                Password = "gooseyloosey",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "mackcloud@pimpdaddy.com",
                    Email = "mackcloud@pimpdaddy.com",
                    PhoneNumber = "5124772125",
                    FirstName = "Jennifer",
                    LastName = "MacLeod",
                    Address = "2504 Far East Blvd.",
                    ZipCode = "78731"
                },
                Password = "rainyday",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "liz@puppy.com",
                    Email = "liz@puppy.com",
                    PhoneNumber = "5124603832",
                    FirstName = "Elizabeth",
                    LastName = "Markham",
                    Address = "7861 Chevy Mace Rd",
                    ZipCode = "78732"
                },
                Password = "ember22",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "mclarence@puppy.com",
                    Email = "mclarence@puppy.com",
                    PhoneNumber = "8174979188",
                    FirstName = "Clarence",
                    LastName = "Martin",
                    Address = "87 Alcedo St.",
                    ZipCode = "77045"
                },
                Password = "lamemartin",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "lamemartin.Martin@aool.com",
                    Email = "lamemartin.Martin@aool.com",
                    PhoneNumber = "8178770705",
                    FirstName = "Gregory",
                    LastName = "Martinez",
                    Address = "8295 Moon Blvd.",
                    ZipCode = "77030"
                },
                Password = "gregory",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "cmiller@mapster.com",
                    Email = "cmiller@mapster.com",
                    PhoneNumber = "8177482602",
                    FirstName = "Charles",
                    LastName = "Miller",
                    Address = "8962 Side St.",
                    ZipCode = "77031"
                },
                Password = "mucky44",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "nelson.Kelly@puppy.com",
                    Email = "nelson.Kelly@puppy.com",
                    PhoneNumber = "5122950953",
                    FirstName = "Kelly",
                    LastName = "Nelson",
                    Address = "2601 Green River",
                    ZipCode = "78703"
                },
                Password = "Tree34",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "jojoe@puppy.com",
                    Email = "jojoe@puppy.com",
                    PhoneNumber = "2143149884",
                    FirstName = "Joe",
                    LastName = "Nguyen",
                    Address = "1249 4th NW St.",
                    ZipCode = "75238"
                },
                Password = "jvb485bg",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "orielly@foxnets.com",
                    Email = "orielly@foxnets.com",
                    PhoneNumber = "2103474912",
                    FirstName = "Bill",
                    LastName = "O'Reilly",
                    Address = "8800 Gringo Drive",
                    ZipCode = "78260"
                },
                Password = "Bobbygirl",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "or@puppy.com",
                    Email = "or@puppy.com",
                    PhoneNumber = "2142369553",
                    FirstName = "Anka",
                    LastName = "Radkovich",
                    Address = "1300 Freaky",
                    ZipCode = "75260"
                },
                Password = "radioactive",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "megrhodes@freezing.co.uk",
                    Email = "megrhodes@freezing.co.uk",
                    PhoneNumber = "5123768733",
                    FirstName = "Megan",
                    LastName = "Rhodes",
                    Address = "4587 Rightfield Rd.",
                    ZipCode = "78707"
                },
                Password = "gopigs",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "erynrice@puppy.com",
                    Email = "erynrice@puppy.com",
                    PhoneNumber = "5123900644",
                    FirstName = "Eryn",
                    LastName = "Rice",
                    Address = "3405 Rio Small",
                    ZipCode = "78705"
                },
                Password = "iloveme",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "jorge@hootmail.com",
                    Email = "jorge@hootmail.com",
                    PhoneNumber = "8178928361",
                    FirstName = "Jorge",
                    LastName = "Rodriguez",
                    Address = "6788 Cotten Street",
                    ZipCode = "77057"
                },
                Password = "565656",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "ra@aoo.com",
                    Email = "ra@aoo.com",
                    PhoneNumber = "5128776930",
                    FirstName = "Allen",
                    LastName = "Rogers",
                    Address = "4965 Rabbit Hill",
                    ZipCode = "78732"
                },
                Password = "treeman",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "o_st-jean@home.com",
                    Email = "o_st-jean@home.com",
                    PhoneNumber = "2104169665",
                    FirstName = "Olivier",
                    LastName = "Saint-Jean",
                    Address = "255 Slap Dr.",
                    ZipCode = "78292"
                },
                Password = "55htrq",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "ss34@puppy.com",
                    Email = "ss34@puppy.com",
                    PhoneNumber = "5123521797",
                    FirstName = "Sarah",
                    LastName = "Saunders",
                    Address = "332 Fish C",
                    ZipCode = "78705"
                },
                Password = "leaves",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "willsheff@email.com",
                    Email = "willsheff@email.com",
                    PhoneNumber = "5124534071",
                    FirstName = "William",
                    LastName = "Sewell",
                    Address = "2365 34st St.",
                    ZipCode = "78709"
                },
                Password = "borbj44",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "sheff44@puppy.com",
                    Email = "sheff44@puppy.com",
                    PhoneNumber = "5125503154",
                    FirstName = "Martin",
                    LastName = "Sheffield",
                    Address = "3886 Road A",
                    ZipCode = "78705"
                },
                Password = "ldiul485",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "johnsmith187@puppy.com",
                    Email = "johnsmith187@puppy.com",
                    PhoneNumber = "2108345875",
                    FirstName = "John",
                    LastName = "Smith",
                    Address = "23 Known Forge Dr.",
                    ZipCode = "78280"
                },
                Password = "kribv75",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "dustroud@mail.com",
                    Email = "dustroud@mail.com",
                    PhoneNumber = "2142370654",
                    FirstName = "Dustin",
                    LastName = "Stroud",
                    Address = "1212 Henrietta Rd",
                    ZipCode = "75221"
                },
                Password = "klavjkb48",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "eric_stuart@puppy.com",
                    Email = "eric_stuart@puppy.com",
                    PhoneNumber = "5128202322",
                    FirstName = "Eric",
                    LastName = "Stuart",
                    Address = "5576 Big Ring",
                    ZipCode = "78746"
                },
                Password = "vkb451",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "peterstump@hootmail.com",
                    Email = "peterstump@hootmail.com",
                    PhoneNumber = "8174584890",
                    FirstName = "Peter",
                    LastName = "Stump",
                    Address = "1300 Kellen Square",
                    ZipCode = "77018"
                },
                Password = "kdsiu4",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "tanner@puppy.com",
                    Email = "tanner@puppy.com",
                    PhoneNumber = "8174614916",
                    FirstName = "Jeremy",
                    LastName = "Tanner",
                    Address = "4347 Palmstead",
                    ZipCode = "77044"
                },
                Password = "klrfbj45",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "taylordjay@puppy.com",
                    Email = "taylordjay@puppy.com",
                    PhoneNumber = "5124772439",
                    FirstName = "Allison",
                    LastName = "Taylor",
                    Address = "467 Nueces St.",
                    ZipCode = "78705"
                },
                Password = "lraggrhb854",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "lraggrhb854.Taylor@aool.com",
                    Email = "lraggrhb854.Taylor@aool.com",
                    PhoneNumber = "5124536618",
                    FirstName = "Rachel",
                    LastName = "Taylor",
                    Address = "345 Shortview Dr.",
                    ZipCode = "78705"
                },
                Password = "alsuib95",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "tee_frank@hootmail.com",
                    Email = "tee_frank@hootmail.com",
                    PhoneNumber = "8178789530",
                    FirstName = "Frank",
                    LastName = "Tee",
                    Address = "5590 Big Dr.",
                    ZipCode = "77004"
                },
                Password = "kd1734",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "tuck33@puppy.com",
                    Email = "tuck33@puppy.com",
                    PhoneNumber = "2148495141",
                    FirstName = "Clent",
                    LastName = "Tucker",
                    Address = "3132 Main St.",
                    ZipCode = "75315"
                },
                Password = "kjdb983",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "avelasco@puppy.com",
                    Email = "avelasco@puppy.com",
                    PhoneNumber = "2144009625",
                    FirstName = "Allen",
                    LastName = "Velasco",
                    Address = "634 W. 4th",
                    ZipCode = "75207"
                },
                Password = "odrb02",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "westj@pioneer.net",
                    Email = "westj@pioneer.net",
                    PhoneNumber = "2148499231",
                    FirstName = "Jake",
                    LastName = "West",
                    Address = "RR 3244",
                    ZipCode = "75323"
                },
                Password = "kndl847",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "louielouie@puppy.com",
                    Email = "louielouie@puppy.com",
                    PhoneNumber = "2145674085",
                    FirstName = "Louis",
                    LastName = "Winthorpe",
                    Address = "2500 Madre Blvd",
                    ZipCode = "78746"
                },
                Password = "lb2394",
                RoleName = "Customer"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "rwood@voyager.net",
                    Email = "rwood@voyager.net",
                    PhoneNumber = "5124569229",
                    FirstName = "Reagan",
                    LastName = "Wood",
                    Address = "447 Westlake Dr.",
                    ZipCode = "78746"
                },
                Password = "drai494",
                RoleName = "Customer"
            });


            //Employee Seed Data
            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "jacobs@bevostunes.com",
                    Email = "jacobs@bevostunes.com",
                    PhoneNumber = "8176663948",
                    FirstName = "Todd",
                    LastName = "Jacobs",
                    Address = "4564 Elm St.",
                    ZipCode = "77003"
                },
                Password = "tj2245",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "rice@bevostunes.com",
                    Email = "rice@bevostunes.com",
                    PhoneNumber = "2148545987",
                    FirstName = "Eryn",
                    LastName = "Rice",
                    Address = "3405 Rio Grande",
                    ZipCode = "75261"
                },
                Password = "ricearoni",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "ingram@bevostunes.com",
                    Email = "ingram@bevostunes.com",
                    PhoneNumber = "5127049017",
                    FirstName = "John",
                    LastName = "Ingram",
                    Address = "6548 La Posada Ct.",
                    ZipCode = "78705"
                },
                Password = "ingram68",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "taylor@bevostunes.com",
                    Email = "taylor@bevostunes.com",
                    PhoneNumber = "2149036025",
                    FirstName = "Albert",
                    LastName = "Taylor",
                    Address = "467 Nueces St.",
                    ZipCode = "75237"
                },
                Password = "TRY563",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "martinez@bevostunes.com",
                    Email = "martinez@bevostunes.com",
                    PhoneNumber = "2105859369",
                    FirstName = "Paul",
                    LastName = "Martinez",
                    Address = "8295 Sunset Blvd.",
                    ZipCode = "78239"
                },
                Password = "party1",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "sheffield@bevostunes.com",
                    Email = "sheffield@bevostunes.com",
                    PhoneNumber = "5124749225",
                    FirstName = "Molly",
                    LastName = "Sheffield",
                    Address = "3886 Avenue A",
                    ZipCode = "78736"
                },
                Password = "longsnores",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "macleod@bevostunes.com",
                    Email = "macleod@bevostunes.com",
                    PhoneNumber = "5124723769",
                    FirstName = "Jenny",
                    LastName = "MacLeod",
                    Address = "2504 Far West Blvd.",
                    ZipCode = "78731"
                },
                Password = "kittys",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "tanner@bevostunes.com",
                    Email = "tanner@bevostunes.com",
                    PhoneNumber = "5129527803",
                    FirstName = "Jared",
                    LastName = "Tanner",
                    Address = "4347 Almstead",
                    ZipCode = "78761"
                },
                Password = "sandman",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "rhodes@bevostunes.com",
                    Email = "rhodes@bevostunes.com",
                    PhoneNumber = "2102520380",
                    FirstName = "Michelle",
                    LastName = "Rhodes",
                    Address = "4587 Enfield Rd.",
                    ZipCode = "78293"
                },
                Password = "puppies",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "stuart@bevostunes.com",
                    Email = "stuart@bevostunes.com",
                    PhoneNumber = "2105415031",
                    FirstName = "Evan",
                    LastName = "Stuart",
                    Address = "5576 Toro Ring",
                    ZipCode = "78279"
                },
                Password = "coolboi",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "chung@bevostunes.com",
                    Email = "chung@bevostunes.com",
                    PhoneNumber = "2107053952",
                    FirstName = "Lauren",
                    LastName = "Chung",
                    Address = "234 RR 12",
                    ZipCode = "78268"
                },
                Password = "listen",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "swanson@bevostunes.com",
                    Email = "swanson@bevostunes.com",
                    PhoneNumber = "5124818542",
                    FirstName = "Ron",
                    LastName = "Swanson",
                    Address = "245 River Rd",
                    ZipCode = "78731"
                },
                Password = "swanbong",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "loter@bevostunes.com",
                    Email = "loter@bevostunes.com",
                    PhoneNumber = "5124650249",
                    FirstName = "Wandavison",
                    LastName = "Loter",
                    Address = "3453 RR 3235",
                    ZipCode = "78732"
                },
                Password = "pottery",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "white@bevostunes.com",
                    Email = "white@bevostunes.com",
                    PhoneNumber = "8175025605",
                    FirstName = "Jabriel",
                    LastName = "White",
                    Address = "12 Valley View",
                    ZipCode = "77045"
                },
                Password = "456789",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "montgomery@bevostunes.com",
                    Email = "montgomery@bevostunes.com",
                    PhoneNumber = "8178817122",
                    FirstName = "Washington",
                    LastName = "Montgomery",
                    Address = "210 Blanco Dr",
                    ZipCode = "77030"
                },
                Password = "python4",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "morales@bevostunes.com",
                    Email = "morales@bevostunes.com",
                    PhoneNumber = "8177529019",
                    FirstName = "Heather",
                    LastName = "Morales",
                    Address = "4501 RR 140",
                    ZipCode = "77031"
                },
                Password = "heckin",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "rankin@bevostunes.com",
                    Email = "rankin@bevostunes.com",
                    PhoneNumber = "5122997370",
                    FirstName = "Martin",
                    LastName = "Rankin",
                    Address = "340 Second St",
                    ZipCode = "78703"
                },
                Password = "rankmark",
                RoleName = "Employee"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "walker@bevostunes.com",
                    Email = "walker@bevostunes.com",
                    PhoneNumber = "2143196301",
                    FirstName = "Lisa",
                    LastName = "Walker",
                    Address = "9 Bison Circle",
                    ZipCode = "75238"
                },
                Password = "walkameter",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "chang@bevostunes.com",
                    Email = "chang@bevostunes.com",
                    PhoneNumber = "2103521329",
                    FirstName = "Gregory",
                    LastName = "Chang",
                    Address = "9003 Joshua St",
                    ZipCode = "78260"
                },
                Password = "pupgang",
                RoleName = "Manager"
            });

            AllUsers.Add(new AddUserModel()
            {
                User = new AppUser()
                {
                    UserName = "gonzalez@bevostunes.com",
                    Email = "gonzalez@bevostunes.com",
                    PhoneNumber = "2142415970",
                    FirstName = "Garth",
                    LastName = "Gonzalez",
                    Address = "103 Manor Rd",
                    ZipCode = "75260"
                },
                Password = "gg2017",
                RoleName = "Employee"
            });





            //create flag to help with errors
            String errorFlag = "Start";

            //create an identity result
            IdentityResult result = new IdentityResult();
            //call the method to seed the user
            try
            {
                foreach (AddUserModel aum in AllUsers)
                {
                    errorFlag = aum.User.Email;
                    result = await Utilities.AddUser.AddUserWithRoleAsync(aum, userManager, context);
                }
            }
            catch (Exception ex)
            {
                throw new Exception("There was a problem adding the user with email: " 
                    + errorFlag, ex);
            }

            return result;
        }
    }
}
